...
class TestContactForm(TestCase):
    ...
    def test_contact_form_fullname_is_required(self):
        form = ContactForm({'name': '', 'email': 'john.doe@email.com', 'content': 'test'})
        self.assertFalse(form.is_valid())
        self.assertIn('name', form.errors.keys())
        self.assertEqual(form.errors['name'][0], 'This field is required.')

    def test_contact_form_email_is_required(self):
        form = ContactForm({'name': 'john doe', 'email': '', 'content': 'test'})
        self.assertFalse(form.is_valid())
        self.assertIn('email', form.errors.keys())
        self.assertEqual(form.errors['email'][0], 'This field is required.')

    def test_contact_form_content_is_required(self):
        form = ContactForm({'name': 'john doe', 'email': 'john.doe@email.com', 'content': ''})
        self.assertFalse(form.is_valid())
